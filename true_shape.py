...
class TrueShapeEngine:
    ...
    def generate_layout(self, parts=None):
        ...
        self.blocks_used = []
        placed_per_block = []
        # R-tree index por bloco para colisão O(log n)
        trees_per_block = []
        ...
        for part_idx, part in enumerate(sorted_parts):
            ...
            for b_idx, block_polys in enumerate(placed_per_block):
                ...
                if result is not None:
                    ...
                    # Reconstruir R-tree com a nova peça
    ...
    def _try_place(self, part, existing, tree):
        ...
        1. STRtree para query espacial O(log n) em vez de O(n)
        2. Prepared geometry para interseção acelerada
        3. 4 rotações (0°, 90°, 180°, 270°) em vez de 2
        4. AABB pre-filter mantido como fast-path
        """
        minx, miny, maxx, maxy = part.bounds
        pw, ph = maxx - minx, maxy - miny

        # Gerar candidatos com rotações
        rotations = [part]
        for angle in [90, 180, 270]:
            rotated = rotate(part, angle, origin='centroid')
            rminx, rminy, rmaxx, rmaxy = rotated.bounds
            rpw, rph = rmaxx - rminx, rmaxy - rminy
            # Só adicionar se a peça rotacionada cabe no bloco
            if rpw <= self.block_w + 0.1 and rph <= self.block_h + 0.1:
                rotations.append(rotated)

        # Step adaptativo baseado na menor dimensão
            ...
            while y + cph <= self.block_h + 0.1:
                ...
                while x + cpw <= self.block_w + 0.1:
                    ...
                    if existing and tree is not None:
                        # STRtree query: retorna índices de candidatos próximos (O(log n))
                        nearby_indices = tree.query(placed)

                        if len(nearby_indices) > 0:
                            # Prepared geometry para teste preciso acelerado
                        ...
                        # Fallback sem tree (primeiro posicionamento)
                        for ep in existing:
